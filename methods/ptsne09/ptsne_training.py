...
def calculate_optimized_p_cond(input_points: tensor,
    ...
    p_cond = get_p_cond(distances, sq_sigmas, diag_mask, eps=eps)
...
def get_p_cond(distances: tensor, sigmas_sq: tensor, mask: tensor, eps) -> tensor:
    """
...
def get_q_joint(emb_points: tensor, dist_func: str, alpha: int, eps) -> tensor:
    """
...
def make_joint(distr_cond: tensor, eps) -> tensor:
    """
...
def loss_function(p_joint: Tensor, q_joint: Tensor, eps) -> Tensor:
    """
...
def fit_ptsne_model(model: torch.nn.Module,
    ...
    for epoch in loop:
        ...
        for list_with_batch in train_dl:
            orig_points_batch = list_with_batch
            ...
            if perplexity is not None:
                ...
                if p_cond_in_batch is None:
                    continue
                p_joint_in_batch = make_joint(p_cond_in_batch, eps=eps)
            ...
            # Multiscale parametric t-SNE
                ...
                for h in range(1, max_entropy):
                    ...
                    p_joint_for_h = make_joint(p_cond_for_h, eps=eps)
            ...
            q_joint_in_batch = get_q_joint(embeddings, "euc", alpha=1, eps=eps)

            # Calculate loss
            loss = loss_function(p_joint_in_batch, q_joint_in_batch, eps=eps)
        ...
        if verbose:
            ...
            loop.set_postfix(loss = f"Avg Loss{average_loss:.4f}")
