...
def p_ij_sym(x, perp, verbose=False):
    ...
    neighbors = np.empty((num_pts, k-1), dtype=np.int32)
...
class ParametricTSNE(nn.Module):
    ...
    def fit(self, training_data,
        ...
        EPS = torch.tensor([1e-10]).to(torch.device('cuda' if self.use_cuda else 'cpu'))
        ...
        for epoch in range(epochs):
            ...
            for batch, data in enumerate(dataloader):
                ...
                q = q_ij(self(features), self.alpha, eps=EPS)
                q = q / q.sum()
