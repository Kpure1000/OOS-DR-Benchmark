...
class CDRTrainer:
    def __init__(self, model, data_matrix, configs, device='cuda', verbose=False):
        self.model = model
    ...
    def encode(self, x):
        ...
        # Pass the input to the model's encode function
    ...
    def _train_begin(self, launch_time_stamp=None):
        ...
        param_template = "Experiment Configurations: \nEpochs: %d Batch Size: %d \n" \
                         "Learning rate: %4f Optimizer: %s\n"

        param_str = param_template % (self.epoch_num, self.batch_size,
    ...
    def train(self, launch_time_stamp=None):
        ...
        for epoch in range(self.start_epoch, self.epoch_num):
            ...
            self._after_epoch(ckp_save_inter, epoch + 1, training_loss, training_loss_history,
                              self.val_inter)
    ...
    def visualize(self, device="cuda"):
    ...
    def build_dataset(self):
        ...
        self.train_loader, self.n_samples = cdr_dataset.get_data_loaders(
            resume_start_epoch, self.data_matrix, self.n_neighbors, self.is_image)
    ...
    def post_epoch(self, ckp_save_inter, epoch, val_inter):
        ...
        if epoch % val_inter == 0:
            ...
            embeddings = self.visualize(device=self.device)
