...
def compute_accurate_knn(flattened_data, k, metric="euclidean"):
    cur_path = None
    ...
    pairwise_distance = get_pairwise_distance(flattened_data, metric, preload=preload)
    ...
    InfoLogger.info("successfully compute accurate neighbor_graph")
    return knn_indices, knn_distances


def compute_knn_graph(all_data, k, metric="euclidean", max_candidates=60, accelerate=False):
    flattened_data = all_data.reshape((len(all_data), np.product(all_data.shape[1:])))

    if not accelerate:
        knn_indices, knn_distances = compute_accurate_knn(flattened_data, k)
        return knn_indices, knn_distances
    ...
    InfoLogger.info("successfully compute approximate neighbor_graph")
    return knn_indices, knn_distances


def get_pairwise_distance(flattened_data, metric, preload=False):
    ...
    InfoLogger.info("successfully compute pairwise distance")
