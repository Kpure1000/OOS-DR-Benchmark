...
class MyTextDataset(Dataset):
    def __init__(self, data_matrix):
        ...
        self.targets = np.zeros(len(data_matrix))
...
class UMAPTextDataset(MyTextDataset):
    ...
    def build_fuzzy_simplicial_set(self, n_neighbors):
        knn_indices, knn_distances = compute_knn_graph(self.data, n_neighbors)
    ...
    def umap_process(self, n_neighbors, embedding_epoch):
        umap_graph, sigmas, rhos = self.build_fuzzy_simplicial_set(n_neighbors)
...
class CDRTextDataset(MyTextDataset):
    def __init__(self, data_matrix):
        MyTextDataset.__init__(self, data_matrix)
...
class UMAPCDRTextDataset(CDRTextDataset):
    def __init__(self, data_matrix):
        CDRTextDataset.__init__(self, data_matrix)
